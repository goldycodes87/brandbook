...
export async function POST(req: NextRequest) {
  ...
  const row: Partial<Insert<'leases'>> = {}
  for (const k of ALLOWED) {
    if (k in body) (row as Record<string, unknown>)[k] = body[k] === '' ? null : body[k]
  }
  ...
  const { data, error } = await supabase.from('leases').insert(row as Insert<'leases'>).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
