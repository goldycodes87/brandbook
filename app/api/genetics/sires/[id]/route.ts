...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updates: Update<'sire_library'> = {}

  const textFields = ['bull_name', 'bull_type', 'breed', 'registration_number', 'naab_code', 'stud', 'photo_url', 'notes', 'epd_source']
  textFields.forEach(f => { if (f in body) (updates as Record<string, unknown>)[f] = body[f] || null })
  ...
  epdFields.forEach(f => { if (f in body) (updates as Record<string, unknown>)[f] = toNum(body[f]) })
