...
export function HealthEventForm({ animalId, onSuccess, onCancel }: HealthEventFormProps) {
  const [eventType, setEventType] = useState<EventType>('treatment')
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: { event_date: new Date().toISOString().slice(0, 10) },
  })
  ...
  const clearDate      = withdrawalDays && eventDate
    ? addDays(new Date(eventDate), withdrawalDays)
    : null
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      ...
      {showDrug && (
        <Field label="Withdrawal days" helper="Override drug default">
          <Input {...register('withdrawal_days')} type="number" min="0" placeholder={String(drug?.withdrawal_days_meat ?? '')} />
        </Field>
      )}
