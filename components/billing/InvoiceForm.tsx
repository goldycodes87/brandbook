...
export function InvoiceForm({ mode, initialData, onSuccess, onCancel }: InvoiceFormProps) {
  ...
  const handleGenerate = useCallback(async () => {
    if (!ownerId || !periodStart || !periodEnd) {
      setGenerateMsg('Select owner, period start, and period end first.')
      return
    }
    setGenerating(true)
    setGenerateMsg('')
    try {
      const res  = await apiPost('/api/billing/generate', { owner_id: ownerId, period_start: periodStart, period_end: periodEnd })
      const json = await res.json()
      if (!res.ok) { setGenerateMsg(json.error ?? 'Generation failed'); return }
      if (json.line_items?.length > 0) {
        setLineItems(json.line_items.map((li: { description: string; amount: number }) => ({
          description: li.description,
          amount: String(li.amount),
        })))
      }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <AccordionSection title="GRAZING CHARGES" defaultOpen>
        ...
        {generateMsg && (
          <ContextBanner tone={generating ? 'neutral' : generateMsg.includes('Found') ? 'success' : 'neutral'}>
            {generateMsg}
          </ContextBanner>
        )}
